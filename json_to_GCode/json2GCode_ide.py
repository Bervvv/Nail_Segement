...
def create_output_path():
    output_dir = os.path.join(args, 'GCode')
...
#     height = data['size']['height']
    # ***change factor below to adjust nail width***
    factor = 1
    # ***change factor above to adjust nail width***
    half_width = width // 2
    if y > half_width:
        y *= factor
    elif y < half_width:
...
def run(args):
    ...
    for label_file in glob.glob(os.path.join(args, '*.json')):
